...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NPAY);
    ...
    if (!settings.contains("nAnonymizeNPayNetworkAmount"))
        settings.setValue("nAnonymizeNPayNetworkAmount", 1000);

    nAnonymizeNPayNetworkAmount = settings.value("nAnonymizeNPayNetworkAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNPayNetworkAmount"))
        SoftSetArg("-anonymizenpaynetworkamount", settings.value("nAnonymizeNPayNetworkAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in npaynetwork.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNPayNetworkAmount:
            return QVariant(nAnonymizeNPayNetworkAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNPayNetworkAmount:
            nAnonymizeNPayNetworkAmount = value.toInt();
            settings.setValue("nAnonymizeNPayNetworkAmount", nAnonymizeNPayNetworkAmount);
            emit anonymizeNPayNetworkAmountChanged(nAnonymizeNPayNetworkAmount);
            break;
        case CoinControlFeatures:
