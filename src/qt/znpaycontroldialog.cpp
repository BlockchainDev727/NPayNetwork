...
#include "ui_znpaycontroldialog.h"
...
std::set<std::string> ZNPayControlDialog::setSelectedMints;
std::set<CMintMeta> ZNPayControlDialog::setMints;

ZNPayControlDialog::ZNPayControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZNPayControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZNPayControlDialog::~ZNPayControlDialog()
{
    delete ui;
}

void ZNPayControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNPayControlDialog::updateList()
{
...
void ZNPayControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNPayControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZNPay_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNPayControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZNPayControlDialog::GetSelectedMints()
{
...
void ZNPayControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
