...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered npaynetwork address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 NPAY.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal npaynetwork:URI (npaynetwork:XyZ...?message=example)
            {
...
static void NotifyzNPAYReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzNPAYReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzNPAYReset.connect(boost::bind(NotifyzNPAYReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzNPAYReset.disconnect(boost::bind(NotifyzNPAYReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->znpayTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
