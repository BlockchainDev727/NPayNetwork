...
#ifndef NPAYNETWORK_ZNPAYWALLET_H
#define NPAYNETWORK_ZNPAYWALLET_H
...
public:
    CzNPAYWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZNPAY(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZNPAY(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //NPAYNETWORK_ZNPAYWALLET_H
