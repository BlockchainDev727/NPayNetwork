...
CzNPAYWallet::CzNPAYWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znpay seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNPAYSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNPAYSeed_deprecated()) {
                    LogPrintf("%s: Updated zNPAY seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znpay seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znpay wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNPAYWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNPAYCount(nCountLastUsed);
    else if (!walletdb.ReadZNPAYCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNPAYWallet::Lock()
{
    seedMaster = 0;
}

void CzNPAYWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNPAYWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNPAY(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNPAYWallet::LoadMintPoolFromDB()
{
...
void CzNPAYWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNPAYWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNPAYWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znpayTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNPAYWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNPAY(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znpayTracker which also adds to database
    pwalletMain->znpayTracker->Add(dMint, true);
...
void CzNPAYWallet::SeedToZNPAY(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNPAYWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNPAYWallet::GenerateDeterministicZNPAY(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNPAYWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNPAYWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
