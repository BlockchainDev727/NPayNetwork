...
CzNPAYTracker::CzNPAYTracker(std::string strWalletFile)
{
...
CzNPAYTracker::~CzNPAYTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNPAYTracker::Init()
{
...
bool CzNPAYTracker::Archive(CMintMeta& meta)
{
...
bool CzNPAYTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNPAYTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNPAYTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNPAYTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzNPAYTracker::GetSerialHashes()
{
...
CAmount CzNPAYTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNPAYTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNPAYTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNPAYTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNPAYTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNPAYTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNPAYTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNPAYTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNPAYTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNPAYTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNPAYTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNPAYTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNPAYTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNPAYTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNPAYTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNPAYTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNPAYTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dznpay from DB\n", __func__, listDeterministicDB.size());
    }
