...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZNPayStake(spend));
    } else {
        ...
        CNPayStake* npayInput = new CNPayStake();
        npayInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(npayInput);
    }
