...
public:
    ...
    bool WriteZNPAYSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNPAYSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNPAYSeed_deprecated(uint256& seed);
    bool EraseZNPAYSeed();
    bool EraseZNPAYSeed_deprecated();

    bool WriteZNPAYCount(const uint32_t& nCount);
    bool ReadZNPAYCount(uint32_t& nCount);
