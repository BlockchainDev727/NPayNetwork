...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.znpayTracker = unique_ptr<CzNPAYTracker>(new CzNPAYTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZNPAY_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZNPAY_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
