...
*Setup instructions for a gitian build of NPAYNETWORK using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the NPAYNETWORK
...
- [Building NPAYNETWORK](#building-npaynetwork)
...
Clone the git repositories for npaynetwork and gitian and then checkout the npaynetwork version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/npaynetwork-crypto/npaynetwork.git
cd npaynetwork
...
Follow the instructions in [doc/release-process.md](release-process.md) in the npaynetwork repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building NPAYNETWORK
----------------

To build NPAYNETWORK (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the npaynetwork repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/npaynetwork/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/npaynetwork-crypto/npaynetwork
...
For example:
```bash
URL=https://github.com/crowning-/npaynetwork.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit npaynetwork=${COMMIT} --url npaynetwork=${URL} ../npaynetwork/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit npaynetwork=${COMMIT} --url npaynetwork=${URL} ../npaynetwork/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit npaynetwork=${COMMIT} --url npaynetwork=${URL} ../npaynetwork/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/npaynetwork-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/npaynetwork-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/npaynetwork-build.assert
```
...
[npaynetwork/gitian.sigs](https://github.com/npaynetwork-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
